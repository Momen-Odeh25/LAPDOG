...
def load_retriever(opt, opt_checkpoint=None):
    if opt.use_file_passages:
        return None, None
    if opt.retriever_model_path.endswith('.ckpt'):
        contriever_encoder = Contriever.from_pretrained('facebook/contriever')
        contriever_weights = torch.load(opt.retriever_model_path, map_location='cpu')['state_dict']
        contriever_weights = {
            k.replace("contriever.", ""): v for k, v in contriever_weights.items()
        }
        contriever_encoder.load_state_dict(contriever_weights)
        retriever_tokenizer = transformers.AutoTokenizer.from_pretrained('facebook/contriever')
    else:
...
def _cast_lapdog_to_precision(lapdog_model, precision):
    if lapdog_model.reader is not None:
        lapdog_model.reader = cast_to_precision(lapdog_model.reader, precision)
    if lapdog_model.retriever is not None and precision == "bf16":
        lapdog_model.retriever = cast_to_precision(lapdog_model.retriever, precision)


def _cast_and_set_attrs_and_send_to_device(model, opt):
    _set_reader_encoder_cfg(model, opt)
    set_dropout(model, opt.dropout)
    _cast_lapdog_to_precision(model, opt.precision)
    model = model.to(opt.device)
    return model


def _load_lapdog_model_state(opt, opt_checkpoint, model, model_dict):
    model_dict = {
    ...
    if opt.load_reader_weights_only:
        reader_model_dict = {
            k.replace("reader.", ""): v for k, v in model_dict.items()
        }
        model.reader.load_state_dict(reader_model_dict)
    else:
...
def load_lapdog_model(dir_path, opt, reset_params=False, eval_only=False):
    ...
    model = _load_lapdog_model_state(opt, opt_checkpoint, model, model_dict)
...
def init_lapdog_model(opt, eval_only):
    reader, reader_tokenizer = load_reader(opt)
...
def load_or_initialize_lapdog_model(opt, eval_only=False):
    """
    ...
    if opt.model_path == "none":
        if not os.path.exists(latest_checkpoint_path):  # Fresh run:
            return init_lapdog_model(opt, eval_only)
        else:  # Resume  run
    ...
    model, optimizer, scheduler, retr_optimizer, retr_scheduler, opt_checkpoint, loaded_step = load_lapdog_model(
...
def save_lapdog_model(model, optimizer, scheduler, retr_optimizer, retr_scheduler, step, opt, dir_path, name):
